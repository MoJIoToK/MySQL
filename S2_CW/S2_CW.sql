...
ALTER TABLE cinema
MODIFY COLUMN genre_id BIGINT UNSIGNED; -- UNSIGNED - только положительные 

ALTER TABLE cinema
ADD FOREIGN KEY(genre_id) REFERENCES genres(id);

-- 6
-- TRUNCATE TABLE genres; -- удаляет все строки сразу навсегда, DELETE - удаляет строку в корзину. Выдаст ошибку, нужно удалить внешнюю связь
...
ALTER TABLE cinema
ADD COLUMN age_category VARCHAR(1); -- добавили новый столбец
...
SELECT id, title, 
	CASE age_category
        ...
        WHEN 'П' THEN 'Подростоквая'
        ELSE 'НЕ УКАЗАНО!'
	END AS "Категория"
FROM cinema;

/*
Задача 4. Выведите id, название фильма, продолжительность, тип в зависимости от продолжительности 
